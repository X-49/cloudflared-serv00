...
get_and_verify_token() {
    while true; do
        ...
        echo "正在验证 token..."
        ${HOME_DIR}/cloudflared-freebsd tunnel --edge-ip-version auto --protocol quic --no-autoupdate run --token $ARGO_AUTH > /dev/null 2>&1 &
        CLOUDFLARED_PID=$!
        sleep 5
...
rotate_log
TZ='Asia/Shanghai' nohup ${HOME_DIR}/cloudflared-freebsd tunnel --edge-ip-version auto --protocol quic --no-autoupdate run --token $ARGO_AUTH >> ${LOG_FILE} 2>&1 &
EOF
